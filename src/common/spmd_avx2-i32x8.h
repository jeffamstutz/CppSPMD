...
struct exec_t
{
  ...
  {
    return exec_t{ _mm256_cmpeq_epi32(_mm256_setzero_si256(),
...
struct vfloat
{
  ...
  // this basically doesn't work because ints get implicitly converted to float
  // anyways. :/:/
...
struct lint
{
  ...
  {
    return vfloat_lref{ ptr +
          _mm_cvtsi128_si32(_mm256_extracti128_si256(_value, 0)) };
  }

  vint_lref operator[](int* ptr) const
  {
    return vint_lref{ ptr +
          _mm_cvtsi128_si32(_mm256_extracti128_si256(_value, 0)) };
  }

  cvint_lref operator[](const int* ptr) const
  {
    return cvint_lref{ ptr +
          _mm_cvtsi128_si32(_mm256_extracti128_si256(_value, 0)) };
  }
...
{
  return vbool{ _mm256_castps_si256(_mm256_cmp_ps(a._value,
                                                  b._value,
                                                  _CMP_EQ_OQ)) };
}
...
{
  return vfloat{ _mm256_blendv_ps(b._value,
                                  a._value,
                                  _mm256_castsi256_ps(cond._value)) };
}
...
vfloat pow(const vfloat& a, const vfloat& b)
{
  ...
  if (fallbackmask != 0)
  {
    ...
    else
    {
      ...
      return vfloat{ _mm256_blendv_ps(nonfallback._value,
                                      _mm256_load_ps(cc),
                                      fallback) };
    }
  }
...
vfloat clamp(const vfloat& v, const vfloat& a, const vfloat& b)
{
  ...
  return vfloat{ _mm256_or_ps(_mm256_and_ps(okmask, v._value),
                              _mm256_or_ps(_mm256_and_ps(lomask, a._value),
                                           _mm256_and_ps(himask, b._value)))};
}
...
struct kernel
{
  ...
  // the execution mask at the current point of varying control flow inside a
  // kernel maintains execution mask of things like spmd_if(), spmd_for(),
  // spmd_break()... generally represents the state of the current loop's
  // execution
  ...
  void spmd_ifelse(const vbool& cond,
                   const IfBody& ifBody,
                   const ElseBody& elseBody);

  template<class WhileCondBody, class WhileBody>
  void spmd_while(const WhileCondBody& whileCondBody,
                  const WhileBody& whileBody);

  template<class ForInitBody, class ForCondBody,
           class ForIncrBody, class ForBody>
  void spmd_for(const ForInitBody& forInitBody,
                const ForCondBody& forCondBody,
                const ForIncrBody& forIncrBody,
                const ForBody& forBody);
...
template<class SPMDKernel, class... Args>
inline decltype(auto) spmd_call(Args&&... args)
{
...
inline void kernel::_init(const exec_t &kernel_exec)
{
...
inline vbool& kernel::store(vbool& dst, const vbool& src)
{
  dst._value =
      _mm256_castps_si256(_mm256_blendv_ps(_mm256_castsi256_ps(dst._value),
                                           _mm256_castsi256_ps(src._value),
                                           _mm256_castsi256_ps(exec._mask)));
  return dst;
}

inline vint& kernel::store(vint& dst, const vint& src)
{
...
inline vfloat& kernel::store(vfloat& dst, const vfloat& src)
{
  return store(std::move(dst), src);
}
...
inline vfloat_lref& kernel::store(vfloat_lref& dst, const vfloat& src)
{
  return store(std::move(dst), src);
}
...
inline vfloat_vref &kernel::store(vfloat_vref &dst, const vfloat &src)
{
  return store(std::move(dst), src);
}
...
inline vint_lref& kernel::store(vint_lref& dst, const vint& src)
{
  int mask = _mm256_movemask_ps(_mm256_castsi256_ps(exec._mask));
  if (mask == 0b11111111)
  {
...
inline vint_vref& kernel::store(vint_vref& dst, const vint& src)
{
  return store(std::move(dst), src);
}
...
inline vfloat kernel::load(const vfloat_lref& src)
{
  int mask = _mm256_movemask_ps(_mm256_castsi256_ps(exec._mask));
  if (mask == 0b11111111)
  {
...
inline vfloat kernel::load(const vfloat_vref& src)
{
...
inline vint kernel::load(const vint_lref& src)
{
  int mask = _mm256_movemask_ps(_mm256_castsi256_ps(exec._mask));
  if (mask == 0b11111111)
  {
...
inline vint kernel::load(const cvint_lref& src)
{
  int mask = _mm256_movemask_ps(_mm256_castsi256_ps(exec._mask));
  if (mask == 0b11111111)
  {
...
inline vint kernel::load(const vint_vref& src)
{
  return vint{ _mm256_mask_i32gather_epi32(_mm256_undefined_si256(),
                                           src._value,
                                           src._vindex,
                                           exec._mask,
                                           4) };
...
template<class IfBody>
inline void kernel::spmd_if(const vbool& cond, const IfBody& ifBody)
{
...
template<class IfBody, class ElseBody>
inline void kernel::spmd_ifelse(const vbool& cond,
                                const IfBody& ifBody,
                                const ElseBody& elseBody)
{
...
template<class WhileCondBody, class WhileBody>
inline void kernel::spmd_while(const WhileCondBody& whileCondBody,
                               const WhileBody& whileBody)
{
  // save old execution mask
  exec_t old_internal_exec = _internal_exec;
...
inline void kernel::spmd_for(const ForInitBody& forInitBody,
                             const ForCondBody& forCondBody,
                             const ForIncrBody& forIncrBody,
                             const ForBody& forBody)
{
...
template<class ForeachBody>
inline void kernel::spmd_foreach(int first, int last,
                                 const ForeachBody& foreachBody)
{
  ...
  // do a partial loop if the loop wasn't a multiple of programCount and the
  // loop isn't just a zombie by now
  if (num_partial_loops > 0 && any(exec))
  {
    // apply mask for partial loop
    exec_t partial_loop_mask =
        exec_t{ _mm256_cmpgt_epi32(_mm256_set1_epi32(num_partial_loops),
                                   programIndex._value) };
...
inline void kernel::spmd_continue()
{
  // this should only be called inside loops. A check for this would be good.
...
inline void kernel::spmd_return()
{
  ...
  // Also, no effort is made to detect that everything is off and do a uniform
  // return.

  // It's not possible to implement other than maybe macro magic, or
  // longjmp/exceptions.
...
template<class UnmaskedBody>
inline void kernel::spmd_unmasked(const UnmaskedBody& unmaskedBody)
{
  // beware: since we don't immediately return when all lanes turn off,
  //         zombie execution is possible.
...
template<class SPMDKernel, class... Args>
inline decltype(auto) kernel::spmd_call(Args&&... args)
{
  return spmd::avx2::spmd_call<SPMDKernel>(std::forward<Args>(args)...);
}

}// namespace avx2
}// namespace spmd

using exec_t = spmd::avx2::exec_t;
using vbool = spmd::avx2::vbool;
using vfloat = spmd::avx2::vfloat;
using vfloat_lref = spmd::avx2::vfloat_lref;
using vfloat_vref = spmd::avx2::vfloat_vref;
using vint = spmd::avx2::vint;
using vint_lref = spmd::avx2::vint_lref;
using cvint_lref = spmd::avx2::cvint_lref;
using vint_vref = spmd::avx2::vint_vref;
using lint = spmd::avx2::lint;
using spmd_kernel = spmd::avx2::kernel;
